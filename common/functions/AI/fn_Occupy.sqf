...
if(count(_Building)>0) then {
	...
	if(( count _list)==0) then {
		_waypoint = [_group,position _BuildingObject,_BuildingObject,_positionIndex,"MOVE","LINE","LIMITED","SAFE",_oncomplete] call A3E_fnc_MoveInBuilding;
	} else {
		_movePos = [(getpos _BuildingObject select 0) + (random 50) - 25,(getpos _BuildingObject select 1) + (random 50) - 25,0];
		_waypoint = [_group,_movePos,"MOVE","LINE","LIMITED","SAFE",_oncomplete] call a3E_fnc_move;
	};
	_waypoint setWaypointTimeout [30, 60, 300];
	[_group,"OCCUPY"] call a3e_fnc_SetTaskState;
} else {
	//No Building near. Do normal patrol duty
	[_group,_markerName] spawn A3E_fnc_Stroll;
};